...
## Data Import
##RFDF <- read.table("filename", sep = ",", header = T)
RFDF.dev <- loksabha2009_ML_Data_RF
RFDF.holdout <- loksabha2014_ML_Data_RF
warnings()

##drop unwanted columns
rp <- c(1,2,3,4)
RFDF.dev <- (RFDF.dev[, -rp])
RFDF.holdout <- (RFDF.holdout[, -rp])

## Lm test to find significant variables 
library(lmtest)
...
#RFDF.dev$`Criminal Case` <- as.numeric(as.factor(RFDF.dev$`Criminal Case`))
## Response Rate
